...
void SomfyIohcCover::program() {
  ...
  auto frames_remove =
      this->build_1w_frames(iohc_cmd::CMD_REMOVE_CONTROLLER, remove_data, sizeof(remove_data), iohc::BROADCAST_ADDR);
  ESP_LOGD(TAG, "PROG: tx CMD_REMOVE_CONTROLLER (0x%02X), %u frame(s)",
           iohc_cmd::CMD_REMOVE_CONTROLLER, static_cast<unsigned>(frames_remove.size()));
  for (auto &frame : frames_remove)
  ...
  // trailer is not. The assembled body (34 bytes) exceeds the 31-byte 1W
  // frame limit, so this is sent as multiple linked fragments -- see the
  // EXPERIMENTAL/UNVERIFIED note on iohc_proto::fragment_1w_frame().
  uint8_t key_data[18];
  ...
  auto frames_key =
      this->build_1w_frames(iohc_cmd::CMD_WRITE_PRIVATE, key_data, sizeof(key_data), iohc::BROADCAST_ADDR, 16);
  ESP_LOGD(TAG, "PROG: tx CMD_WRITE_PRIVATE (0x%02X), %u frame(s) (key omitted)",
           iohc_cmd::CMD_WRITE_PRIVATE, static_cast<unsigned>(frames_key.size()));
  for (auto &frame : frames_key)
    this->hub_->transmit_packet(frame, static_cast<uint8_t>(this->repeat_count_));
...
void SomfyIohcCover::send_1w_command(uint16_t main_param) {
  ...
  auto frames = this->build_1w_frames(iohc_cmd::CMD_EXECUTE, data, sizeof(data), iohc::BROADCAST_ADDR);
...
std::vector<std::vector<uint8_t>> SomfyIohcCover::build_1w_frames(uint8_t cmd, const uint8_t *data,
  ...
  // Assemble the full logical body: ctrl1 || dest(3) || src(3) || cmd || data
  // || seq(2) || mac(6) -- everything a single (unfragmented) 1W frame would
  // carry between ctrl0 and the trailing CRC.
  std::vector<uint8_t> body;
  body.reserve(1 + 6 + 1 + data_len + 2 + 6);

  // CtrlByte1: 1W frames carry no Start/End framing bits (0x00).
  body.push_back(0x00);

  // Destination node ID (3 bytes, big-endian)
  body.push_back(static_cast<uint8_t>((dest_node >> 16) & 0xFF));
  body.push_back(static_cast<uint8_t>((dest_node >> 8) & 0xFF));
  body.push_back(static_cast<uint8_t>(dest_node & 0xFF));

  // Source node ID (3 bytes, big-endian)
  body.push_back(static_cast<uint8_t>((this->node_id_ >> 16) & 0xFF));
  body.push_back(static_cast<uint8_t>((this->node_id_ >> 8) & 0xFF));
  body.push_back(static_cast<uint8_t>(this->node_id_ & 0xFF));
  ...
  body.push_back(static_cast<uint8_t>(sequence >> 8));
  body.push_back(static_cast<uint8_t>(sequence & 0xFF));
  ...
  for (int i = 0; i < 6; i++) {
    body.push_back(mac[i]);
  }

  if (body.size() > iohc_proto::IOHC_1W_MAX_BODY) {
    // io-homecontrol's 5-bit length field caps a single frame at 31 body
    // bytes. Larger commands (e.g. pairing's 34-byte CMD_WRITE_PRIVATE) are
    // split into multiple linked frames instead of overflowing the size
    // field -- see the EXPERIMENTAL/UNVERIFIED note on
    // iohc_proto::fragment_1w_frame(): the fragment linking scheme is a
    // best-effort guess, unconfirmed against real hardware.
    ESP_LOGW(TAG, "1W frame cmd=0x%02X body=%u B exceeds the 31-byte 1W limit; fragmenting "
             "(EXPERIMENTAL/UNVERIFIED, see PR notes)",
             cmd, static_cast<unsigned>(body.size()));
  }

  auto frames = iohc_proto::fragment_1w_frame(body.data(), body.size());
  ESP_LOGD(TAG, "1W frame cmd=0x%02X dst=0x%06X seq=%u body=%u B -> %u physical frame(s)", cmd, dest_node,
           sequence, static_cast<unsigned>(body.size()), static_cast<unsigned>(frames.size()));
  return frames;
}
